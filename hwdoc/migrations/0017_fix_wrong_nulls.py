...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm['hwdoc.Equipment'].objects.filter(purpose__isnull=True).update(purpose='')
        orm['hwdoc.Equipment'].objects.filter(comments__isnull=True).update(comments='')
        orm['hwdoc.ServerManagement'].objects.filter(username__isnull=True).update(username='')
        orm['hwdoc.ServerManagement'].objects.filter(password__isnull=True).update(password='')
    ...
    def backwards(self, orm):
        ...
        orm['hwdoc.ServerManagement'].objects.filter(username='').update(username=None)
        orm['hwdoc.ServerManagement'].objects.filter(password='').update(password=None)
